...
void delay_OTA(unsigned long msec = 0)
{
  ...
  unsigned long stopm = millis();
  while (stopm > millis() - msec)
  {
    client.loop();
    ArduinoOTA.handle();
    yield();    
...
enum State { lazy, selectr, waitforled, selnohit, selhit, cmdsend, endwait };
...
void setup() {
  ...
  pinMode(PIN_IN1, PINMODE1);
  pinMode(PIN_IN2, PINMODE2);
  pinMode(PIN_IN3, PINMODE3); 
  pinMode(PIN_IN4, PINMODE4); 
...
void loop() {
  if (!client.loop()) {
    ...
    #endif
    static const unsigned int pinin[4] = { PIN_IN1, PIN_IN2, PIN_IN3, PIN_IN4 };
    static const uint32_t mask = (1 << PIN_IN1) | (1 << PIN_IN2) | (1 << PIN_IN3) | (1 << PIN_IN4);
    static uint32_t old_in_all = mask;
    static unsigned long last_select = millis() - 11;
    static uint32_t in_all = old_in_all;
    if (millis() - last_select > 10) in_all = ((GPI & 0xffff) | ((GP16I & 1) << 16)) & mask; 
    if (old_in_all != in_all ) {
      old_in_all = in_all; 
      changed = true; 
      #ifdef DEBUG
        changed2 = true;
    ...
    switch(s) {
      ...
      case selectr:
        ...
        if (changed) {
          ...
          int curSel = 0;
          int cnt = 0;
          for (int i=0; i<4; i++) if (((in_all >> pinin[i]) & 1) == 0) {curSel = i+1; cnt++;}
          if ((cnt == 4 && cur_RolNo == 0) || (cnt == 1 && curSel == cur_RolNo)) s = selhit;
          else s = selnohit;
        } else if (millis() - last_event > WAITLEDDURATION) s = selnohit;
        break;
      case selnohit:
